...
public class UserDaoImpl implements UserDao {
	...
	public UserPojo validateUserNum(UserPojo user) {
		...
		try {
			...
			if (rs.next()) {
				logUser = new UserPojo(rs.getInt(USER_ID), rs.getString(USER_NAME), rs.getString(EMAIL),
						rs.getLong(MOBILE_NUMBER), rs.getString("role"));
			}
	...
	public List<UserPojo> showusers() {
		...
		try {
			...
			rs = pstmt.executeQuery();
			while (rs.next()) {
				users = new UserPojo(rs.getInt(USER_ID), rs.getString(USER_NAME), rs.getString(EMAIL),
